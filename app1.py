...
# ---------------------------------------------------------
def authenticate():
    """Renders the password input form and handles authentication."""
    st.title("🔒 Production Dashboard Access")
    st.markdown("### Please enter the password to view the report.")

    # Use a form to ensure inputs are captured on button press
    with st.form("login_form"):
        user_password = st.text_input(
            "Password",
            type="password",
            help="Enter your access password here."
        )
        submitted = st.form_submit_button("Enter")

        if submitted:
            if user_password == PASSWORD:
                st.session_state[LOGIN_STATUS_KEY] = True
                st.rerun() # Rerun the script to display the main content
            else:
...
if LOGIN_STATUS_KEY not in st.session_state:
    st.session_state[LOGIN_STATUS_KEY] = False

# Only run dashboard if logged in
if not st.session_state[LOGIN_STATUS_KEY]:
    authenticate()
else:
